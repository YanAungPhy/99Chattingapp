...
import com.chatapp.nineninechatapp.BuildConfig;
...
public class RecorderUtils {
    ...
    public static String getRecorderPath() {
        ...
        // File sdDir = Environment.getExternalStorageDirectory(); original
        String  filepath = "/data/user/0/"+ BuildConfig.APPLICATION_ID+"/files";
        File eis = new File(filepath + "/RecordVideo/");
        if (!eis.exists()) {
            eis.mkdir();
        }
        return filepath + "/RecordVideo/";
    }
    ...
    public static String getShareThumbPath(String name) {
        if (TextUtils.isEmpty(name)) {
            return null;
        }
        ...
        String fileName = name.substring(name.lastIndexOf("/") + 1);
        String[] temp = fileName.split("\\.");
        String out =temp[0] + "_thumb%03d.jpg";
        return out;
    }
    ...
    public static String getShareVideoFileByTime() {
        String filePath ="VID_" + new SimpleDateFormat("yyyyMMdd_HHmmssSSS").format(new Date()) + ".mp4";
        return filePath;
    }
